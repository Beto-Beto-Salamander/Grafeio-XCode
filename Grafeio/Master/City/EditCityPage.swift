...
struct EditCityPage: View {
    ...
    var body: some View {
        ...
        if CityEdit.isSuccess {
            return AnyView(CityPage().environmentObject(AuthUser))
        }
        else {
            return AnyView(EditViewCity(Cities: Cities))
        }
    }
}

struct EditViewCity : View {
    @EnvironmentObject var AuthUser: UserAuth
    @EnvironmentObject var CityEdit: EditCity
    ...
    var CInitTextF : some View {
        HStack(){
            ...
            TextField(Cities!.City_Initial, text : self.$CityInitial)
    ...
    var CNameTextF : some View {
        HStack(){
            ...
            TextField(Cities!.City_Name, text : self.$CityName)
                .foregroundColor(.black)
                .padding()
                .font(.headline)
                .frame(width: 380, height: 50, alignment: .topLeading)
    ...
    var ProvTextF : some View {
        HStack(){
            ...
            TextField(Cities!.Province_Name, text : self.$Prov)
                .foregroundColor(.black)
                .padding()
                .font(.headline)
                .frame(width: 380, height: 50, alignment: .topLeading)
        ...
        VStack{
            ...
            Spacer()
                ...
                let primaryButton = Alert.Button.default(Text("Yes")) {
                    CityEdit.City_ID = Cities!.City_ID
                    self.CityEdit.editCity(provid: Prov,
                                           cname: CityName,
                                           cinitial: CityInitial)
...
/*struct EditCityPage_Previews: PreviewProvider {
